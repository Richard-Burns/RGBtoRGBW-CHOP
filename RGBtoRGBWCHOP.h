...
class RGBtoRGBWCHOP : public CHOP_CPlusPlusBase
{
public:
	RGBtoRGBWCHOP(const OP_NodeInfo* info);
	virtual ~RGBtoRGBWCHOP();
...
private:
	...
	RGBW ConvertToRGBW(float r, float g, float b) {
		...
		int Mi = std::max(Ri, std::max(Gi, Bi));
		int mi = std::min(Ri, std::min(Gi, Bi));
		...
		int Ro = floor(((K * Ri) - Wo));
		int Go = floor(((K * Gi) - Wo));
		int Bo = floor(((K * Bi) - Wo));


		float fr = (float)Ro / 255;
		float fg = (float)Go / 255;
		float fb = (float)Bo / 255;
		float fw = (float)Wo / 255;

		RGBW result;
		result.r = clip(fr,0.0,1.0);
		result.g = clip(fg,0.0,1.0);
		result.b = clip(fb,0.0,1.0);
		result.w = clip(fw,0.0,1.0);
